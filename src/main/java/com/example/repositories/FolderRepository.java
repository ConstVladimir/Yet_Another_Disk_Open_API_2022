...
public interface FolderRepository extends CrudRepository<SystemItemFolder, String> {
    ...
    void setItem (String id, String url, Timestamp date, String parentId, Long sizeIt, String children);

    @Modifying
    @Query("UPDATE SYSTEM_ITEM_FOLDER SET UPDATE_DATE=:date, SIZE_IT=:sizeIt WHERE ID=:id")
    void updateParentFolder (String id, Timestamp date, Long sizeIt);

    @Modifying
    @Query("UPDATE SYSTEM_ITEM_FOLDER SET UPDATE_DATE=:date, SIZE_IT=:sizeIt, CHILDREN=:children WHERE ID=:id")
    void updateParentFolderAndAddItemToChildren (String id, Timestamp date, Long sizeIt, String children);
